...
function createBot() {
  ...
  // Bot restart bo'lguncha foydalanuvchilarni saqlab turadi (statistika/rassilka uchun)
  const users = new Set();
  ...
  bot.action('start', (ctx) => {
    users.add(ctx.from.id);
    return ctx
      .editMessageText('Kerakli obunani quyidagi ro‘yxatdan tanlashingiz mumkin 👇', mainMenu)
      .catch(() => {});
  });
  ...
  bot.action('admin_broadcast', (ctx) => {
    ...
    ctx.answerCbQuery();
    isBroadcasting = true;
    ctx.reply("📢 Rassilka matnini yuboring. Matn yuborilgandan keyin u barcha foydalanuvchilarga boradi.");
  });
